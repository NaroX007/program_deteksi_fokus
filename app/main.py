"""
main.py - Sistem Deteksi Fokus Anak (PRODUCTION MODE)
=====================================================
Mode production yang terhubung ke sistem permainan RFID dan web dashboard.
...
- Pencatatan ke SQLite lokal + sinkronisasi Firebase
...
# ============================================================
HEADLESS_MODE = False
STANDALONE_MODE = False           # Production mode — terhubung ke web/permainan
...
def run_ai_loop(anak_id, session_id):
    ...
    def monitor_session():
        """
        Monitor status sesi dari web.
        Berhenti saat sesi ditandai selesai oleh sistem permainan.
        """
...
def main():
    print("=" * 60)
    print("=== SISTEM ANALISIS FOKUS AKTIF (PRODUCTION MODE) ===")
    print("=" * 60)
    print(f"[INFO] Mode: Production (Terhubung ke Web/Permainan)")
    print(f"[INFO] Process every: {PROCESS_EVERY_N_FRAMES} frames")
    print(f"[INFO] Camera: {CAMERA_WIDTH}x{CAMERA_HEIGHT}")
    print(f"[INFO] Min off-task duration (arah): {MIN_OFF_TASK_DURATION}s | (kepala): 1.0s")
    print("=" * 60)
